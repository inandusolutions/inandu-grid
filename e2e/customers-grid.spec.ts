...
/**
 * Drives the demo site's flagship grid (`#customers-grid` in src/app/app.component.html — 100 static
 * rows, `lang="en"`, `filter`/`selectable`/`exportable` all on, paginated) through a real browser,
 ...
 *
 * This grid sits in the "Grid basics" panel, the demo's default tab, so no tab click is needed.
 */
...
test('sorting by a column changes row order', async ({ page }) => {
  ...
  // City, Role, Id, Company, Contact, Active, Since, Revenue — Company is the 5th cell (index 4).
  const firstCompanyCell = () => grid.locator('tbody tr.inandu-row').first().locator('td').nth(4);
  const sortButton = grid.getByRole('button', { name: 'Sort by Company' });
...
test('free-text search narrows the visible rows', async ({ page }) => {
  ...
  await grid.getByPlaceholder('Search').fill('Alfreds');
  await expect(rows).toHaveCount(1);
  await expect(rows.first()).toContainText('Alfreds');

  await grid.getByPlaceholder('Search').fill('');
...
test('pagination advances to the next page and updates the page label', async ({ page }) => {
  ...
  await expect(label).toHaveText('Page 1 of 10');
  await grid.getByRole('button', { name: 'Next page' }).click();
  await expect(label).toHaveText('Page 2 of 10');
});
