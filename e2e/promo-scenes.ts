import { test } from '@playwright/test';

/**
 * Not an assertion suite. Each test drives the demo site through one "look what it does" gesture
 * at a watchable pace. The `promo` project in playwright.config.ts records video for every test in
 * this file; `scripts/make-gifs.mjs` turns each recording into `docs/gifs/<test name>.gif`. Run it
 * all with `npm run gifs` (or just the recording with `npm run gifs:record`).
 *
 * The demo is tabbed and inactive panels are `hidden`, so a scene outside the default "Grid basics"
 * tab opens its own tab first. Selectors that aren't certain (group rows) are guarded, so a scene
 * that can't find its target still records the grid rather than failing the run.
 */

const READY = 'inandu-grid#customers-grid tbody tr.inandu-row';

/** Loads the page and waits for the flagship grid's first rows. */
async function open(page: import('@playwright/test').Page, tab?: string): Promise<void> {
  await page.goto('/');
  await page.locator(READY).first().waitFor();
  if (tab) {
    await page.locator(tab).click();
    await page.waitForTimeout(400);
  }
  await page.waitForTimeout(600);
}

test('search', async ({ page }) => {
  await open(page);

  const box = page.locator('inandu-grid#customers-grid').getByPlaceholder('Search');
  await box.click();
  for (const ch of 'london') {
    await box.press(ch);
    await page.waitForTimeout(170);
  }
  await page.waitForTimeout(1400);
  for (let i = 0; i < 6; i++) {
    await box.press('Backspace');
    await page.waitForTimeout(140);
  }
  await page.waitForTimeout(900);
});

test('multi-sort', async ({ page }) => {
  await open(page);

  const grid = page.locator('inandu-grid#customers-grid');
  await grid.getByRole('button', { name: 'Sort by City' }).click();
  await page.waitForTimeout(850);
  await grid.getByRole('button', { name: 'Sort by City' }).click();
  await page.waitForTimeout(850);
  await grid.getByRole('button', { name: 'Sort by Company' }).click({ modifiers: ['Shift'] });
  await page.waitForTimeout(1400);
});

test('grouping', async ({ page }) => {
  await open(page);

  const grid = page.locator('inandu-grid#customers-grid');
  const header = grid.locator('thead th', { hasText: 'Role' });
  const zone = grid.getByText(/Drag a column header here/i);
  const h = await header.boundingBox();
  const z = await zone.boundingBox();
  if (!h || !z) throw new Error('grouping: header or drop zone not found');

  await page.mouse.move(h.x + h.width / 2, h.y + h.height / 2);
  await page.mouse.down();
  await page.mouse.move(h.x + h.width / 2 + 12, h.y + 4, { steps: 6 });
  await page.mouse.move(z.x + z.width / 2, z.y + z.height / 2, { steps: 24 });
  await page.waitForTimeout(400);
  await page.mouse.up();
  await page.waitForTimeout(1600);

  const group = grid.locator('tbody tr.inandu-group-row, tbody tr.inandu-group, tbody tr[class*="group"]').first();
  if (await group.count()) {
    await group.click();
    await page.waitForTimeout(1300);
  }
});

test('row-selection', async ({ page }) => {
  await open(page);

  const grid = page.locator('inandu-grid#customers-grid');
  const rows = grid.locator('tbody tr.inandu-row');
  for (const i of [0, 2, 4]) {
    await rows.nth(i).locator('input[type="checkbox"]').check();
    await page.waitForTimeout(430);
  }
  await page.waitForTimeout(650);
  await grid.locator('thead input[type="checkbox"]').check();
  await page.waitForTimeout(1300);
});

test('inline-edit', async ({ page }) => {
  await open(page, '#tab-editing');

  const grid = page.locator('inandu-grid#edit-demo-grid');
  await grid.scrollIntoViewIfNeeded();
  await page.waitForTimeout(400);
  const row = grid.locator('tbody tr.inandu-row').first();
  await row.getByRole('button', { name: 'Edit' }).click();
  await page.waitForTimeout(700);
  const price = row.locator('td').nth(3).locator('.inandu-cell-edit-input');
  await price.click();
  await price.fill('149.90');
  await page.waitForTimeout(800);
  await row.locator('.inandu-row-actions button').first().click();
  await page.waitForTimeout(1400);
});

test('tree-data', async ({ page }) => {
  await open(page, '#tab-tree');

  const grid = page.locator('inandu-grid#tree-grid');
  await grid.scrollIntoViewIfNeeded();
  await page.waitForTimeout(500);
  // Icon-only <button class="inandu-tree-toggle">; force past the icon-font actionability wait
  // so a slow webfont on CI doesn't stall the recording.
  const toggles = grid.locator('tbody tr.inandu-tree-row button.inandu-tree-toggle');
  const n = Math.min(await toggles.count(), 3);
  for (let i = 0; i < n; i++) {
    await toggles.nth(i).click({ force: true, timeout: 4000 }).catch(() => {});
    await page.waitForTimeout(850);
  }
  await page.waitForTimeout(800);
});

test('virtual-scroll', async ({ page }) => {
  await open(page, '#tab-scale');

  const grid = page.locator('inandu-grid#virtual-scroll-grid');
  await grid.scrollIntoViewIfNeeded();
  await page.waitForTimeout(500);
  const b = await grid.boundingBox();
  if (!b) throw new Error('virtual-scroll: grid not found');

  await page.mouse.move(b.x + b.width / 2, b.y + b.height / 2);
  for (let i = 0; i < 12; i++) {
    await page.mouse.wheel(0, 500);
    await page.waitForTimeout(110);
  }
  await page.waitForTimeout(600);
  await grid.getByRole('button', { name: 'Sort by Price' }).click();
  await page.waitForTimeout(1000);
});
