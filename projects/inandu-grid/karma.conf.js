...
//
// Wired in via angular.json's inandu-grid "test" architect target (`options.karmaConfig`) so we can
...
// single-Chrome default so `npm run test:lib` behaves exactly as before; pass `--browsers=...` on
// the CLI to run against Firefox/Safari instead (see CONTRIBUTING.md).
...
module.exports = function (config) {
  config.set({
    ...
    coverageReporter: {
      dir: require('path').join(__dirname, '../../coverage/inandu-grid'),
      subdir: '.',
    ...
    // Local `npm run test:lib` is unaffected — it still uses the plain 'ChromeHeadless' default.
    customLaunchers: {
      ChromeHeadlessCI: {
        base: 'ChromeHeadless',
